...
impl CodeCellTraceContext {
    ...
    /// runtime completion.
    pub fn record_initial_response(&self, response: &Value) {
        let CodeCellTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        append_with_context_best_effort(
    ...
    pub fn record_ended(&self, response: &Value) {
        let CodeCellTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        append_with_context_best_effort(
...
fn code_cell_status_for_runtime_response(response: &Value) -> CodeCellRuntimeStatus {
    if response.get("Yielded").is_some() {
        CodeCellRuntimeStatus::Yielded
    } else if response.get("Terminated").is_some() {
        CodeCellRuntimeStatus::Terminated
    } else if response
        .get("Result")
        .and_then(|result| result.get("error_text"))
        .is_some_and(|error_text| !error_text.is_null())
    {
