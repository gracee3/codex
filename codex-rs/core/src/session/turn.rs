...
use crate::tools::router::map_mcp_tool_infos;
...
pub(crate) async fn run_turn(
    ...
    let _skill_name_counts_lower = skills_outcome
        .as_ref()
    ...
    let _skills_outcome = Some(turn_context.turn_skills.outcome.as_ref());
...
pub(crate) fn build_prompt(
    ...
    let exposed_tool_names = model_visible_tool_names(&tools);
    let exposed_tool_name_refs = exposed_tool_names
        .iter()
        .map(String::as_str)
        .collect::<HashSet<_>>();
    tools.extend(
        collect_unavailable_called_tools(&input, &exposed_tool_name_refs)
            .into_iter()
            .map(unavailable_tool_spec),
    );
...
fn model_visible_tool_names(tools: &[ToolSpec]) -> HashSet<String> {
        ...
        match tool {
            ...
            ToolSpec::Namespace(namespace) => {
                for tool in &namespace.tools {
                    ...
                    names.insert(format!("{}{}", namespace.name, tool.name));
                }
            }
            ToolSpec::ToolSearch { .. }
            | ToolSpec::LocalShell {}
...
fn unavailable_tool_spec(tool_name: ToolName) -> ToolSpec {
    ToolSpec::Function(ResponsesApiTool {
        name: tool_name.display(),
        description:
            "Placeholder for a previously used MCP tool that is not currently available."
                .to_string(),
        strict: false,
        defer_loading: None,
...
pub(crate) async fn built_tools(
    ...
    let mapped_mcp_tools = mcp_tools.as_ref().map(map_mcp_tool_infos);

    Ok(Arc::new(ToolRouter::from_config(
        &turn_context.tools_config,
        ToolRouterParams {
            mcp_tools: mapped_mcp_tools
                .as_ref()
                .map(|inputs| inputs.mcp_tools.clone()),
            parallel_mcp_tools: mapped_mcp_tools
                .as_ref()
                .map(|inputs| inputs.parallel_mcp_tools.clone()),
            tool_namespaces: mapped_mcp_tools
                .as_ref()
