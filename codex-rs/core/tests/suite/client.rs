...
async fn resume_replays_image_tool_outputs_with_detail() {
    ...
    let custom_call_id = "legacy-image-tool-call";
    let rollout = vec![
        RolloutLine {
        ...
        RolloutLine {
            ...
            item: RolloutItem::ResponseItem(ResponseItem::CustomToolCall {
                ...
                name: "legacy_image_tool".to_string(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn omits_apps_guidance_as_developer_message_for_chatgpt_auth() {
    skip_if_no_network!();
    let server = MockServer::start().await;
    ...
    assert!(
        ...
        "did not expect apps guidance in a developer message, got {:?}",
...
/// value on both Windows and Unix. Note that this test must also work in
/// restricted CI environments, so PATH seems like the safest choice.
