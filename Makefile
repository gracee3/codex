...
new-tt-release:
	PATCH_BRANCHES="fork/maint fork/dev-build-speedups fork/tt-runtime-contract fork/app-server-rollout" RELEASE_BRANCH_PREFIX="releases/tt/" $(FORK_SCRIPT) new-release $(if $(TAG),--tag $(TAG),) $(if $(PUSH),--push,)

new-tt-alpha-release:
	PATCH_BRANCHES="fork/maint fork/dev-build-speedups fork/tt-runtime-contract fork/app-server-rollout" RELEASE_BRANCH_PREFIX="releases/tt/" $(FORK_SCRIPT) new-release --alpha $(if $(TAG),--tag $(TAG),) $(if $(PUSH),--push,)
...
tt-source-install:
	@CODEX_SOURCE_PATCH_BRANCHES="fork/maint fork/dev-build-speedups fork/tt-runtime-contract fork/app-server-rollout" CODEX_SOURCE_RELEASE_BRANCH_PREFIX="releases/tt/" "$(SOURCE_INSTALL_SCRIPT)" --dir "$(SOURCE_DIR)" --install-dir "$(INSTALL_DIR)" --tag "$(VERSION)"
